...
one raises `PicoPieError`, so use the [headless PNG helpers](#headless-use-the-png-helpers-instead)
(below) there.
...
picopie.show(part)                      # opens a window, runs until you close it
picopie.show(body, holes, struts)       # several objects, each auto-colored
```
...
```python
picopie.render_png(part, "part.png", size=(1920, 1080))
picopie.render_png([body, holes], "scene.png")     # a list -> palette colors
```
